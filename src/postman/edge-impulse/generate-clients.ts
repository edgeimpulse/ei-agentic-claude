...
function getBaseUrl(collection: any) {
  const baseVar = (collection.variable || []).find((v: any) => v.key === 'baseUrl');
  return baseVar?.value || 'https://studio.edgeimpulse.com/v1';
}

function buildUrlParts(endpoint: any, baseUrl: string) {
  const urlObj = endpoint.request.url || {};
  const raw = (urlObj.raw || '').replace(/{{baseUrl}}/g, baseUrl);
  const basePath = raw.split('?')[0];
  const pathParams = (urlObj.variable || []).map((v: any) => v.key);
  const queryParams = (urlObj.query || []).map((q: any) => q.key);
  return { basePath, pathParams, queryParams };
}

function generateClient(endpoint: any, baseUrl: string) {
