 ...
 */
export async function run_pipelines(params: any, apiKey: string) {
  const pathParams: string[] = ["organizationId","pipelineId"];
  const queryParams: string[] = ["ignoreLastSuccessfulRun"];

  let url = `https://studio.edgeimpulse.com/v1/api/organizations/:organizationId/pipelines/:pipelineId/run`;
