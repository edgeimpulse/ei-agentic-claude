 ...
 */
export async function whitelabel_admin_update_organization_export(params: any, apiKey: string) {
  const pathParams: string[] = ["organizationId","innerOrganizationId","exportId"];
  const queryParams: string[] = [];

  let url = `https://studio.edgeimpulse.com/v1/api/organizations/:organizationId/whitelabel/organizations/:innerOrganizationId/exports/:exportId`;
  ...
  const hasBody = !['GET', 'HEAD'].includes('PUT') && Object.keys(bodyParams).length > 0;
