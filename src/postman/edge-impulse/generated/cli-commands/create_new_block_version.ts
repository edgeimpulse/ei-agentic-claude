...
export function addCreate_new_block_versionCommand(program: Command) {
  program.command('create-new-block-version')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'create-new-block-version' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
