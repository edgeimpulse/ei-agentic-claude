...
export function addUpdate_transformation_jobCommand(program: Command) {
  program.command('update-transformation-job')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'update-transformation-job' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
