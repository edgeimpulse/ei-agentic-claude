...
export function addRemove_all_samples_by_categoryCommand(program: Command) {
  program.command('remove-all-samples-by-category')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'remove-all-samples-by-category' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
