...
export function addDelete_theme_by_idCommand(program: Command) {
  program.command('delete-theme-by-id')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'delete-theme-by-id' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
