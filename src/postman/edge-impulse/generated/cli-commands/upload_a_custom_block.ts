...
export function addUpload_a_custom_blockCommand(program: Command) {
  program.command('upload-a-custom-block')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'upload-a-custom-block' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
