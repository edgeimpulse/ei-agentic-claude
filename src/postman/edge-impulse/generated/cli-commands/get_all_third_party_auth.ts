...
export function addGet_all_third_party_authCommand(program: Command) {
  program.command('get-all-third-party-auth')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'get-all-third-party-auth' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
