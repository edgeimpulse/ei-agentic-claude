...
export function addGet_deploy_blocksCommand(program: Command) {
  program.command('get-deploy-blocks')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'get-deploy-blocks' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
