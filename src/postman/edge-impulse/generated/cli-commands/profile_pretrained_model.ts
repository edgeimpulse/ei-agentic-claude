...
export function addProfile_pretrained_modelCommand(program: Command) {
  program.command('profile-pretrained-model')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'profile-pretrained-model' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
