...
export function addAdd_current_user_to_a_projectCommand(program: Command) {
  program.command('add-current-user-to-a-project')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'add-current-user-to-a-project' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
