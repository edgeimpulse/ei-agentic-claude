...
export function addUpdate_theme_faviconCommand(program: Command) {
  program.command('update-theme-favicon')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'update-theme-favicon' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
