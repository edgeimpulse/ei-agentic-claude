...
export function addBuild_organizational_on_device_modelCommand(program: Command) {
  program.command('build-organizational-on-device-model')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'build-organizational-on-device-model' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
