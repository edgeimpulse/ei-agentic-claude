import { Command } from 'commander';
  import { white_label_information } from '../white_label_information';

export function addWhite_label_informationCommand(program: Command) {
  program.command('white-label-information')
    .description('Auto-generated command for white_label_information')
    .requiredOption('--api-key <apiKey>', 'Edge Impulse API key')
    .option('--params <params>', 'JSON string of parameters')
    .action(async (opts) => {
      try {
        const params = opts.params ? JSON.parse(opts.params) : {};
        const res = await white_label_information(params, opts.apiKey);
        console.log(JSON.stringify(res, null, 2));
      } catch (e) {
        console.warn(`Warning: Command 'white-label-information' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
