...
export function addUpdate_third_party_authCommand(program: Command) {
  program.command('update-third-party-auth')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'update-third-party-auth' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
