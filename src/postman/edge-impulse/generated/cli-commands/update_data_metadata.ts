...
export function addUpdate_data_metadataCommand(program: Command) {
  program.command('update-data-metadata')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'update-data-metadata' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
