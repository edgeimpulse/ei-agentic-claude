...
export function addUpdate_current_userCommand(program: Command) {
  program.command('update-current-user')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'update-current-user' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
