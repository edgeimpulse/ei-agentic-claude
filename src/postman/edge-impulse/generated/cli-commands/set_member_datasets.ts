...
export function addSet_member_datasetsCommand(program: Command) {
  program.command('set-member-datasets')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'set-member-datasets' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
