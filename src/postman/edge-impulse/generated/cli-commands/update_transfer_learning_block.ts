...
export function addUpdate_transfer_learning_blockCommand(program: Command) {
  program.command('update-transfer-learning-block')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'update-transfer-learning-block' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
