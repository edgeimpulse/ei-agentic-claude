...
export function addDeployment_targets_data_sourcesCommand(program: Command) {
  program.command('deployment-targets-data-sources')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'deployment-targets-data-sources' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
