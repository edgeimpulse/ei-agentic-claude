...
export function addExport_keras_blockCommand(program: Command) {
  program.command('export-keras-block')
    ...
    .action(async (opts) => {
      try {
        ...
        console.warn(`Warning: Command 'export-keras-block' may need extension - ${e instanceof Error ? e.message : e}`);
        process.exit(0);
      }
    });
}
