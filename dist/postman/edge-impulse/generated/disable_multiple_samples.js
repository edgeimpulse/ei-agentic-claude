...
export async function disable_multiple_samples(params, apiKey) {
    ...
    const res = await fetch(`https://studio.edgeimpulse.com/api/:projectId/raw-data/batch/disable-samples?category=<string>&labels=<string>&filename=<string>&maxLength=<integer>&minLength=<integer>&minFrequency=<number>&maxFrequency=<number>&signatureValidity=<string>&includeDisabled=<string>&ids=<string>`, {
        method: 'POST',
        headers: {
            'x-api-key': apiKey,
